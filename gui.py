...
class Game:
    ...
    def select(self, pos:tuple):
        ...
        if self.__selected:
            try:
                ...
                piece = ''
                if chess.Move.from_uci(move.uci+'q') in self.__board:
                    piece = self.promote()
                self.__board.push_uci(move.uci+piece)
                self.__draw_board()
            finally:
                self.__selected = None
                return
    ...
    def promote(self):
        nrect = pg.Rect(self.__surface.get_width()//8, self.__surface.get_height()//4, self.__surface.get_width()//8*6, self.__surface.get_height()//2)
        pg.draw.rect(self.__surface, (128,128,128), nrect, border_radius=10)
        while True:
